...
def calculate_gpa(student: Student, courses: List[Course]) -> float:
    ...
    for course in courses:
        if student.id in course.grades:
            grade = course.grades[student.id] # Assuming Student class has a method to get the student's grade for a course
            grade_points = Course.convert_grade_to_points(grade)  # Using static method to convert grade to points
            course_hours = course.hours  # Assuming Course class has an attribute 'hours' representing the course's credit hours
